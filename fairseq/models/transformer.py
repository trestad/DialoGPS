...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(
        ...
        if replace_data is not None and replace_length is not None and replace_data is not None:
            if token_embeddings is None:
                token_embeddings = self.embed_tokens(src_tokens)
            x = encoder_embedding = self.embed_scale * token_embeddings

            for i in range(x.size(0)):
                x[i][replace_index[i]:replace_index[i] + replace_length[i]] = replace_data[i][:replace_length[i]]
            if self.embed_positions is not None:
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def forward(
        ...
        if not features_only:
            x_after = self.output_layer(x)
        return x_after, extra, x
