...
def _main(args, output_file):
    ...
    #print(models[0].history_encoder.embed_tokens.weight.shape)
    # with open("embed_inter.txt",'w') as f:
    #    for word, v in zip(tgt_dict.symbols, models[0].history_encoder.embed_tokens.weight):
    #        f.write(word)
    #        f.write(' ')
    #        f.write(' '.join(str(round(x,2)) for x in v.detach().numpy()))
    #        f.write('\n')
    #exit()
    ...
    for sample in progress:
        ...
        # with open('embed_base_dd.txt','wb') as vocab_file:
        #     print('writing vocab vectors')
        #     for i in range(models[0].encoder.embed_tokens.weight.data.size(0)):
        #         vocab_file.write(str(tgt_dict[i]).encode())
        #         vocab_file.write(b' ')
        #         vocab_file.write(' '.join(str(round(x.item(),3)) for x in models[0].encoder.embed_tokens.weight.data[i]).encode())
        #         vocab_file.write(b'\n')
        # exit()
